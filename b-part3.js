...
function submit() {
  ...
  // avoids having multiple hashed words.
  ...
  if (/^\b[a-zA-Z]+\b$/i.test(toHashTag)) {
    ...
    // Search for the word given by the user:
    // If the word is found, a # is append at the beginning
    let replacement = mainText.textContent.replace(
      regex,
      `<a href="${twitter}">#${toHashTag}</a>`
    );

    // The code refreshes the content of the text:
